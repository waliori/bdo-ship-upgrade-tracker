...
export const BOARDS = [
	...
	{ id: 'ship', section: 'sea', title: TT('Best ship'), icon: '⛵', unit: TT('pts'), min: 1,
		...
		detail: d => {
			...
			return `${gameName(b.ship)}${sets ? ` · ${sets}` : ''}${b.levels ? ` · ${T('+{n} in all', { n: b.levels })}` : ''}`;
		},
		face: faceShip },
	{ id: 'fleet', section: 'sea', title: TT('Largest fleet'), icon: '🚢', unit: TT('hulls'), min: 2,
		desc: TT('hulls owned, across your setups and your inventory'), how: TT('Own a second hull on the Ship tab.'),
		note: TT('Hulls, counted once each: every hull with a saved setup, with parts fitted by hand, or sitting in your Inventory. Two of the same hull are one hull here.'),
		value: d => d.fleet.n, detail: d => d.fleet.hulls.slice(0, 3).map(h => gameName(h)).join(', '), face: d => faceItems(d.fleet.hulls.slice(0, 4)) },
	{ id: 'sailor', section: 'sea', title: TT('Best sailor'), icon: '🧭', unit: TT('pts'), min: 1001,
		desc: TT('the strongest hand aboard: level first, then growths'), how: TT('Hire a sailor on the Ship tab.'),
		note: TT('Your single best sailor: their level times a thousand, plus their growth percentages added up. Level decides it; the growths only break a tie.'),
		value: d => (d.crew.best ? d.crew.best.score : 0),
		detail: d => (d.crew.best ? `${d.crew.best.name} · ${gameName(d.crew.best.type)} · ${T('Lv {n}', { n: d.crew.best.lv })}${d.crew.best.sum ? ` · ${T('stats {n}', { n: d.crew.best.sum })}` : ''}` : ''),
	...
	{ id: 'crew', section: 'sea', title: TT('Largest crew'), icon: '👥', unit: TT('sailors'), min: 1,
		desc: TT('sailors hired across every roster'), how: TT('Hire sailors on the Ship tab.'),
		note: TT('Sailors on the roster, however they are seated.'),
	...
	{ id: 'silver', section: 'runs', title: TT('Most silver from runs'), icon: '💰', unit: TT('silver'), min: 1,
		...
		note: TT('Silver over every run logged. The career tally is used where it is larger than the last sixty runs, since a save keeps only those.'),
		value: d => d.runs.silver, detail: d => `${d.runs.n === 1 ? T('{n} run', { n: d.runs.n }) : T('{n} runs', { n: d.runs.n })} · ${d.runs.trades === 1 ? T('{n} trade', { n: d.runs.trades }) : T('{n} trades', { n: d.runs.trades })}`, face: () => null },
	{ id: 'runs', section: 'runs', title: TT('Most runs sailed'), icon: '🌊', unit: TT('runs'), min: 1,
		desc: TT('runs logged, over the whole career'), how: TT('Log a run on the Barter tab.'),
		note: TT('Runs logged over the career, by the same reckoning as the silver.'),
		value: d => d.runs.n, detail: d => `${T('{n} stops', { n: d.runs.stops })} · ${T('{n} trades', { n: d.runs.trades })}`, face: () => null },
	{ id: 'bestday', section: 'runs', title: TT('Best single run'), icon: '☀', unit: TT('silver net'), min: 1,
		desc: TT('the most silver netted in one run'), how: TT('Log a run on the Barter tab.'),
		note: TT('The most silver netted in a single run — what it paid, less what it cost — over the last sixty.'),
		value: d => (d.runs.best ? d.runs.best.net : 0), detail: d => (d.runs.best ? d.runs.best.day : ''), face: () => null },
	{ id: 'quests', section: 'quests', title: TT('Most quests done'), icon: '✦', unit: TT('quests'), min: 1,
		desc: TT('claims over the career, on the Quests tab'), how: TT('Claim a quest on the Quests tab.'),
		note: TT('Every quest claim over the career, counting a quest claimed twice as two.'),
		value: d => d.quests.n, detail: d => T('{n} different', { n: d.quests.distinct }), face: () => null },
	{ id: 'hunts', section: 'quests', title: TT('Most sea monsters hunted'), icon: '🦈', unit: TT('hunts'), min: 1,
		desc: TT('from the hunting quests done'), how: TT('Claim a hunting quest on the Quests tab.'),
		note: TT('Claims of the hunting quests only, which is what the app can see of a monster killed.'),
		value: d => d.quests.huntsN, detail: d => Object.keys(d.quests.hunts).slice(0, 3).map(k => gameName(monsterName(k))).join(', '),
		face: d => (Object.keys(d.quests.hunts).length ? { kind: 'monsters', keys: Object.keys(d.quests.hunts).slice(0, 4) } : null) },
	{ id: 'ships', section: 'yard', title: TT('Most ships built'), icon: '⚒', unit: TT('ships'), min: 1,
		desc: TT('hulls made in the Workshop'), how: TT('Craft a hull in the Workshop.'),
		note: TT('Hulls recorded as made in the Workshop.'),
		value: d => d.yard.ships, detail: d => Object.keys(d.yard.shipsMade).slice(0, 2).map(s => gameName(s)).join(', '), face: d => faceItems(Object.keys(d.yard.shipsMade).slice(0, 4)) },
	{ id: 'crafts', section: 'yard', title: TT('Most things made'), icon: '🔨', unit: TT('crafts'), min: 1,
		desc: TT('everything crafted in the Workshop'), how: TT('Craft anything in the Workshop.'),
		note: TT('Everything recorded as made in the Workshop, hulls and parts and materials alike.'),
		value: d => d.yard.crafts, detail: d => (d.yard.parts ? T('{n} ship parts', { n: d.yard.parts }) : ''), face: () => null },
	{ id: 'luck', section: 'yard', title: TT('Luckiest at the anvil'), icon: '🎲', unit: TT('% success'), min: 1,
		desc: TT('successes per attempt, over ten or more tries'), how: TT('Record ten enhancement attempts in the Workshop.'),
		note: TT('Successes per attempt at the anvil, as a percentage, and only once ten attempts are recorded — three lucky tries are not a record.'),
		value: d => (d.yard.tries >= 10 ? Math.round((d.yard.wins / d.yard.tries) * 100) : 0),
		detail: d => (d.yard.tries ? T('{wins} of {tries} attempts', { wins: d.yard.wins, tries: d.yard.tries }) : ''), face: () => null },
	{ id: 'charts', section: 'charts', title: TT('Cartographer'), icon: '✎', unit: TT('points'), min: 1,
		desc: TT('points in the traces drawn on the chart'), how: TT('Draw a trace on the Map tab.'),
		note: TT('Points in the traces drawn on the chart, so a long coast counts more than a short one.'),
		value: d => d.charts.points, detail: d => `${d.charts.traces === 1 ? T('{n} trace', { n: d.charts.traces }) : T('{n} traces', { n: d.charts.traces })} · ${d.charts.routes === 1 ? T('{n} route', { n: d.charts.routes }) : T('{n} routes', { n: d.charts.routes })}`, face: () => null },
	{ id: 'hold', section: 'charts', title: TT('Fullest hold'), icon: '📦', unit: TT('units'), min: 1,
		desc: TT('units in the Inventory, all kinds together'), how: TT('Add stock on the Inventory tab.'),
		note: TT('Units in the Inventory, every kind added together. Silver and Crow Coin are left out; they would drown everything else.'),
		value: d => d.stock.units, detail: d => T('{n} kinds of thing', { n: d.stock.items }), face: () => null }
];

/** The sections the boards are grouped under on the tab. */
export const SECTIONS = [
	{ id: 'sea', icon: '⚓', title: TT('The sea') },
	{ id: 'runs', icon: '⇄', title: TT('The runs') },
	{ id: 'quests', icon: '✦', title: TT('Quests & hunts') },
	{ id: 'yard', icon: '⚒', title: TT('The yard') },
	{ id: 'charts', icon: '✎', title: TT('Charts & hold') }
];

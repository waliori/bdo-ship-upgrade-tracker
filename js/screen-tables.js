...
function tableHTML(id, stack) {
	...
	const rows = t.levels.map((l, i) => {
		...
		return `<tr><td>+${i} → +${i + 1}</td>
			...
			${sure ? '' : `<td class="num lit">${pct(chanceAt(l, at))}<small>${T('at {n}', { n: at })}</small></td>
	...
	const head = `<tr><th>${T('Attempt')}</th><th class="num" title="${T('At no failstack')}">${T('Rate at 0')}</th>${sure ? '' : `
		<th class="num">${T('At your stack')}</th>
		<th class="num" title="${T('The stack at which the chance reaches 70%; every stack up to it adds a tenth of the base rate, every stack past it a fiftieth')}">${T('70% at')}</th>
		<th class="num" title="${T('The stack at which the chance reaches 90%, where it stops')}">${T('90% at')}</th>
		<th class="num" title="${T('Fails before the next attempt cannot fail: the Agris Essence meter')}">${T('Sure after')}</th>`}
		<th class="num">${esc(gameName(t.material))}</th>${yellow ? `<th class="num" title="${T('Cron Stones to keep the level on a failure')}">${T('Cron')}</th>` : ''}
		<th class="num" title="${T('Max durability lost on a failure')}">${T('Durability')}</th>${sure ? '' : `<th class="num" title="${T('Stones for a perfect enhancement, where the game offers one')}">${T('Perfect')}</th>`}</tr>`;
	const note = sure ? T('Every attempt succeeds; the stones per level are the whole cost.')
		: yellow ? T('A failure takes a level as well as durability unless Cron Stones hold it. The rate at 0 is the base; the game quotes each level at the stack in the lit column.')
			: T('A failure keeps the level and costs durability.');
	return `<section class="enh-tables-one"><h3>${esc(t.label)} <span class="map-courses-credit">${parts === 1 ? T('{n} part', { n: parts }) : T('{n} parts', { n: parts })}</span></h3>
...
export function openTables(stack = null) {
	const host = openDialog(`
		<h2>${T('The enhancement tables')}</h2>
		<p class="dialog-copy">${T('All 70 enhanceable ship parts follow one of seven tables, read off BDOCodex. Each failstack adds a tenth of the base rate until the chance reaches 70%, then a fiftieth, and 90% is the ceiling; every failure stores an Agris Essence, and a full meter cannot fail. The Workshop prices each attempt from these at the stack you carry.')}</p>
		<label class="dialog-label">${T('Light every level at {input} stacks', { input: `<input class="field" type="text" inputmode="numeric" data-stack placeholder="${T('your failstack')}" value="${stack === null ? '' : stack}" style="max-width:130px">` })}</label>

...
export const CHANGES = [
	{
		...
		notes: [
			...
			'A <b>Barter</b> tab plans a run from what is aboard. The hold is listed good by good, weighed against the ship as fitted and its overload ceiling, with a count to change on each line. Then a run for <b>silver</b>, on today’s board: every chain the board allows is listed — a land good bought ashore, or a good already aboard, and the islands that take it up rung by rung to a [Level 7] — and the ones ticked are sailed one after the other, nearest first, every attempt at each rung, the goods no rung ahead takes left in storage at the nearest wharf when the hull needs the room, and the [Level 7]s sold at the island that pays them; what is bought ashore, left at a wharf and carried home is listed with what it would sell for. Or a run for <b>a material</b>: its ladder counted against the hold, the first thing missing named, the islands for each rung nearest-first and the refreshes it will take — the run the whole table allows at best until the board is known, and the tab says so. Either draws itself on the chart in one press. The Route panel’s per-level load composer is gone — the hold here is the real one.',

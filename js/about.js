...
export const CHANGES = [
	{
		...
		notes: [
			...
			'<b>And the counts are read by a small network, taught on slots nobody had to label.</b> No OCR engine is fetched for a storage at all \u2014 an engine is the wrong tool for eight-pixel writing over a drawing, and so, it turned out, were templates of the game\u2019s figures: a template is one rendering, and the same window captured by a desktop that scales its screen is another. A stack of 103 over a crate came back as 1,103, because the edge of the crate really is an upright. What reads it now is what reads house numbers off street photographs: a convolutional network run along the whole line and trained with CTC, so nothing has to say where one figure ends and the next begins. It was taught on a third of a million <i>made-up</i> slots \u2014 the game\u2019s own font, pulled out of the client, over the app\u2019s own icons, then blurred, rescaled and recompressed every way a screenshot gets \u2014 and on no real ones, which is what makes the real ones a test: four screenshots, 369 slots, 238 of them numbered, <b>all read and none wrong</b>; shrunk to three fifths, doubled, blurred or recompressed to a rough JPEG there is still no wrong count it was sure of \u2014 what gets harder to read gets marked, not guessed. Fifty thousand weights, ten milliseconds a slot, plain JavaScript. A reading it is not sure of is written in as its best, marked \u26a0, with the corner of the slot beside it.',
			'<b>Scrolled shots are one storage.</b> Shoot a screenful, scroll, shoot again, and the last row of one is the first row of the next \u2014 which used to count twelve crystals as twenty-four. The rows two shots share are now found by their pictures, named or not, and counted once; the table says how many. A row of one thing repeated is not taken as proof, because two rows of dynamite look exactly like one row shot twice. The last row of a storage, the one with two things in it and seven bare slots, is no longer trimmed off for having faint borders.',
			...
			'<b>A run buys only what the Market has.</b> Oni wrote in with a run that told him to load a land good the Central Market had none of: the app priced a chain\u2019s first good at the Market\u2019s last price and never asked whether any were for sale. The relay was already bringing the listed count back with every price; now it is used. With land goods <i>bought ashore</i>, a run buys no more of a good than are listed, a chain whose first good nobody is selling does not start \u2014 its card says <i>none on the Market</i> before it is ticked, and the run says so in words, offering <i>from my storage</i> where you keep some \u2014 and <i>Before casting off</i> shows how many are listed beside each thing to buy. A count the Market would not confirm this time holds nothing back, since stock moves by the minute; and the Market is asked again every half hour while the page is open, not only when it loads.'
		]
	},
	{
		date: '2026-09-14',

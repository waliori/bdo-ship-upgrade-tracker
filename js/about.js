...
export const CHANGES = [
	{
		...
		notes: [
			...
			'What is behind a door is <b>said, not hidden</b>. The Barter tab leaves a chain that climbs through a shut island out of the run and out of the runs worth sailing, and a line above the list says how many and what opens them. A material’s island chips show the shut ones locked with the count beside them, rather than quietly dropping where the thing is dealt. The Map still draws the island and its card says what opens it, but no pin is lit there for something on your list. And a forecast folds through the islands you can actually reach — a rung dealt in two places is priced at the open one — so where none is open the answer is the door itself: <i>locked, 2,520 more barters open the Wandering Merchant’s Ship</i>.',
			'<b>The numbers about you are typed once.</b> The barter count, the level, the Parley, the vouchers and the Value Pack were entered on To Get — the one screen that is not about the sea — and read everywhere else, with a second copy of two of them on the Barter tab. They sit beside the pouch now, in the strip that follows you across every tab, folded into one line that still says the lot: <i>480 barters · Artisan 5 · 4+3 draws</i>, and under it what the next threshold opens and when the lists refill. One press opens the fields, and the choice is remembered. The first mate’s ten per cent comes along, with who to seat for it when nobody is.',
			'<b>How many sailors are out</b>, in the masthead. A count of the browsers with the tracker open at this moment, and in the tooltip how many have ever opened it. It needs no sign-in and keeps no address: a random token the browser makes for itself and two timestamps, which is why it says browsers and not people.'
		]
	},
	{
		date: '2026-09-10',
